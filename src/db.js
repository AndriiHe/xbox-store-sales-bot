...
const { mergeMap, tap, count, catchError } = require('rxjs/operators');
...
class Db {
  ...
  transaction(...queries) {
    ...
    return from(this.pool.connect()).pipe(
      ...
      tap(connection => rxpg = new Db(connection)),
      mergeMap(() => rxpg.query('BEGIN')),
      ...queries.map(query => mergeMap(prevResult => query(rxpg, prevResult))),
      count(),
      mergeMap(() => rxpg.query('COMMIT')),
      catchError(err => {
